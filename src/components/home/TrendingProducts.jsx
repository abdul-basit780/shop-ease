...
import { ShoppingCart, Heart, Star, TrendingUp, ArrowUpRight } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 animate-fade-in">
            <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4">
              Trending <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 to-secondary-600">Now</span>
            </h2>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          {products.length > 0 && (
            <Link href="/products?trending=true">
              <button className="hidden md:inline-flex items-center px-6 py-3 text-primary-600 hover:text-primary-700 font-semibold border-2 border-primary-600 rounded-xl hover:bg-primary-50 transition-all duration-300 transform hover:scale-105">
                View All
              </button>
            </Link>
          )}
            ...
            {products.map((product, idx) => (
              <Link key={product.id} href={`/products/${product.id}`} className="group animate-scale-in" style={{ animationDelay: `${idx * 100}ms` }}>
                <div className="relative bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-2xl transition-all duration-500 cursor-pointer transform hover:-translate-y-2 border border-gray-100">
                  {/* Trending Rank Badge */}
                  <div className="absolute top-3 left-3 z-10 bg-gradient-to-r from-primary-600 to-secondary-600 text-white w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shadow-lg">
                    {idx + 1}
                  </div>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                      <div className="absolute bottom-4 left-4 right-4 flex items-center gap-2">
                        <button
                          ...
                          className="flex-1 flex items-center justify-center space-x-2 px-3 py-2 bg-white text-primary-600 rounded-lg hover:bg-primary-50 font-semibold transition-all transform translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 disabled:opacity-50"
                        >
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={(e) => handleAddToCart(e, product)}
                        disabled={product.stock === 0}
                        className="p-2 rounded-lg bg-gradient-to-r from-primary-600 to-secondary-600 text-white hover:shadow-lg transition-all transform hover:scale-105 disabled:opacity-50"
                      >
        ...
        {products.length > 0 && (
          <div className="text-center mt-12 md:hidden">
            <Link href="/products?trending=true">
              <button className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-primary-600 to-secondary-600 text-white font-semibold rounded-xl hover:shadow-lg transform hover:scale-105 transition-all">
                View All Trending
              </button>
            </Link>
          </div>
